...
namespace VuonVietXuStore
{
    public partial class UC_Welcomeback : UserControl
    {
        ...
        private void LoadDashboardData()
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                try
                {
                    ...
                    // Thêm cột trạng thái giả lập vào DataTable
                    dtHistory.Columns.Add("Trạng Thái", typeof(string));
                    foreach (DataRow row in dtHistory.Rows)
                    {
                        row["Trạng Thái"] = "Đã thu";
                    }
        ...
        // 4. XỬ LÝ SỰ KIỆN DUYỆT ĐƠN HÀNG GIẢ LẬP
        private void dgvChoXacNhan_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && dgvChoXacNhan.Columns[e.ColumnIndex].Name == "Duyet")
            {
                ...
                // Nạp lại dữ liệu để làm mới Dashboard
                LoadDashboardData();
            }
        }
